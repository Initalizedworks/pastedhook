...
{
    if (!isHackActive() || effect_glow::g_EffectGlow.drawing || chams_attachment_drawing || disable_visuals || CE_BAD(LOCAL_E) || (!enable && !no_hats && !no_arms && !blend_zoom && !arms_chams && !local_weapon_chams /*&& !(hacks::backtrack::chams && hacks::backtrack::isBacktrackEnabled)*/))
