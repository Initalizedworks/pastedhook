...
/* TODO check if this still causes crashes with catbots */
...
/*
    ...
    if (ipc::peer && ipc::peer->memory)
        ipc::peer->memory->peer_user_data[ipc::peer->client_id].target_name = potential_targets[idx].first;
#endif
*/
...
static InitRoutine init(
    []()
    {
    namesteal.installChangeCallback([](settings::VariableBase<int> &var, int after) {
    ...
    force_name.installChangeCallback([](settings::VariableBase<std::string> &var, std::string after) {
        if (!GetForcedName(forced_name, g_ISteamUser->GetSteamID()))
            return;
